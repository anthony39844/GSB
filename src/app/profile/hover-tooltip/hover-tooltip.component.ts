...
export class HoverTooltipComponent {
  ...
  ngOnInit() {
    if (this.type === 'item' && this.info != 0) {
      ...
      this.name = data.name;
      this.route = `https://ddragon.leagueoflegends.com/cdn/14.24.1/img/item/${this.info}.png`;
      ...
      this.description = data.description.replace(/<(?!br\s*\/?)[^>]*>/gi, ''); // Removes all tags except <br>
      this.name = data.name;
      this.route = `https://ddragon.leagueoflegends.com/cdn/14.24.1/img/spell/${data.icon}`;
    }
  }
}
